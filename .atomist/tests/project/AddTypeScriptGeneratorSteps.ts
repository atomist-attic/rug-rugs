...
When("AddTypeScriptGenerator generatorName is MyNewGenerator, description is Description of MyNewGenerator for" +
    " AddTypeScriptGenerator should add a new TypeScript generator to an existing Rug Archive project",
    (p, world) => {
        const psworld = world as ProjectScenarioWorld;
        const editor = psworld.editor("AddTypeScriptGenerator");

        psworld.editWith(editor, { generatorName: "MyNewGenerator", description: "Description of MyNewGenerator" });
    });

Then("fileExists atomist editors MyNewGenerator ts for AddTypeScriptGenerator should add a new TypeScript generator" +
    " to an existing Rug Archive project", (p, world) => {
        return p.fileExists(".atomist/generators/MyNewGenerator.ts");
    });

Then("fileContains atomist editors MyNewGenerator ts Generator MyNewGenerator for AddTypeScriptGenerator should" +
    " add a new TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", '@Generator("MyNewGenerator"');
    });

Then("fileContains atomist editors MyNewGenerator ts description for AddTypeScriptGenerator should add a new" +
    " TypeScript generator to an existing Rug Archive project", (p, world) => {
        const description = "Description of MyNewGenerator";
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", '"' + description + '"');
    });

Then("fileContains atomist editors MyNewGenerator ts class MyNewGenerator for AddTypeScriptGenerator should add a" +
    " new TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", "class MyNewGenerator");
    });

Then("fileContains atomist editors MyNewGenerator ts new MyNewGenerator for AddTypeScriptGenerator should add a new" +
    " TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", "new MyNewGenerator()");
    });

Then("not result fileContains atomist editors MyNewGenerator ts TypeScriptGenerator for AddTypeScriptGenerator should" +
    " add a new TypeScript generator to an existing Rug Archive project", (p, world) => {

        return !p.fileContains(".atomist/generators/MyNewGenerator.ts", "TypeScriptGenerator");
    });

Then("not result fileContains atomist editors MyNewGenerator ts sample TypeScript generator used by for" +
    " AddTypeScriptGenerator should add a new TypeScript generator to an existing Rug Archive project",
    (p, world) => {
        return !p.fileContains(".atomist/generators/MyNewGenerator.ts", "sample TypeScript generator used by");
    });

Then("not result fileContains atomist editors MyNewGenerator ts typeScriptGenerator for AddTypeScriptGenerator" +
    " should add a new TypeScript generator to an existing Rug Archive project",
    (p, world) => {
        return !p.fileContains(".atomist/generators/MyNewGenerator.ts", "typeScriptGenerator");
    });

Then("fileExists atomist tests project MyNewGeneratorTest ts for AddTypeScriptGenerator should add a new" +
    " TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileExists(".atomist/tests/project/MyNewGeneratorSteps.ts");
    });

Then("fileContains atomist tests project MyNewGeneratorTest ts scenario MyNewGenerator for AddTypeScriptGenerator" +
    " should add a new TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains(".atomist/tests/project/MyNewGeneratorTest.feature", "Scenario: MyNewGenerator");
    });

Then("not result fileContains atomist tests project MyNewGeneratorTest ts TypeScriptGenerator for" +
    " AddTypeScriptGenerator should add a new TypeScript generator to an existing Rug Archive project",
    (p, world) => {
        return !p.fileContains(".atomist/tests/project/MyNewGeneratorSteps.ts", "TypeScriptGenerator");
    });

Then("fileExists README md for AddTypeScriptGenerator should add a new TypeScript generator to an existing Rug" +
    " Archive project", (p, world) => {
        return p.fileExists("README.md");
    });

Then("fileContains README md MyNewGenerator for AddTypeScriptGenerator should add a new TypeScript generator" +
    " to an existing Rug Archive project", (p, world) => {
        return p.fileContains("README.md", "### MyNewGenerator");
    });

Then("fileContains README md Description of MyNewGenerator for AddTypeScriptGenerator should add a new TypeScript" +
    " generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains("README.md", "Description of MyNewGenerator");
    });

Then("fileContains README md This Rug has no prerequisites for AddTypeScriptGenerator should add a new TypeScript" +
    " generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains("README.md", "This Rug has no prerequisites.");
    });

Then("fileContains README md projectName Yes Name of project to be created for AddTypeScriptGenerator should add" +
    " a new TypeScript generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains("README.md", "`projectName` | Yes | | Name of project to be created");
    });

Then("fileContains README md MyNewGenerator example for AddTypeScriptGenerator should add a new TypeScript" +
    " generator to an existing Rug Archive project", (p, world) => {
        return p.fileContains("README.md", "-l MyNewGenerator ");
    });

Then("fileContains README md Explain what your generator does here for AddTypeScriptGenerator should add a new" +
    " TypeScript generator to an existing Rug Archive project", (p, world) => {

        return p.fileContains("README.md", "Explain what your generator does here.");
    });

Given("a file named .atomist/manifest.yml for AddTypeScriptGenerator should add a new TypeScript generator even" +
    " if no README", (p) => {
...
Given("a file named .atomist/package.json for AddTypeScriptGenerator should add a new TypeScript generator even if" +
    " no README", (p) => {
        p.addFile(".atomist/package.json", `{"dependencies":{"@atomist/rug":"0.12.0"}}`);
    });

When("AddTypeScriptGenerator generatorName is MyNewGenerator, description is Description of MyNewGenerator for" +
    " AddTypeScriptGenerator should add a new TypeScript generator even if no README",
    (p, world) => {
        const psworld = world as ProjectScenarioWorld;
        const editor = psworld.editor("AddTypeScriptGenerator");
        psworld.editWith(editor, { generatorName: "MyNewGenerator", description: "Description of MyNewGenerator" });
...
Then("fileExists atomist editors MyNewGenerator ts for AddTypeScriptGenerator should add a new TypeScript generator" +
    " even if no README", (p, world) => {
        return p.fileExists(".atomist/generators/MyNewGenerator.ts");
    });

Then("fileContains atomist editors MyNewGenerator ts Generator MyNewGenerator for AddTypeScriptGenerator should" +
    " add a new TypeScript generator even if no README", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", '@Generator("MyNewGenerator"');
    });

Then("fileContains atomist editors MyNewGenerator ts description for AddTypeScriptGenerator should add a new" +
    " TypeScript generator even if no README", (p, world) => {
        const description = "Description of MyNewGenerator";
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", '"' + description + '"');
    });

Then("fileContains atomist editors MyNewGenerator ts class MyNewGenerator for AddTypeScriptGenerator should add a" +
    " new TypeScript generator even if no README", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", "class MyNewGenerator");
    });

Then("fileContains atomist editors MyNewGenerator ts new MyNewGenerator for AddTypeScriptGenerator should add a" +
    " new TypeScript generator even if no README", (p, world) => {
        return p.fileContains(".atomist/generators/MyNewGenerator.ts", "new MyNewGenerator()");
    });

Then("not result fileContains atomist editors MyNewGenerator ts TypeScriptGenerator for AddTypeScriptGenerator" +
    " should add a new TypeScript generator even if no README", (p, world) => {
        return !p.fileContains(".atomist/generators/MyNewGenerator.ts", "TypeScriptGenerator");
    });
